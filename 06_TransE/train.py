...
from dataset import TransEDataset
import torch
...
def train(args, model, dataset, device):
    ...
    data_loader = DataLoader(
        ...
        collate_fn=TransEDataset.collate_fn,
        num_workers=20,
        shuffle=True,
    )
    epochs = args.epochs
...
def parse_args():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="8_TransR/model_save/TransE-model-bern2.pth",
        help="Path to the dataset",
    )
    parser.add_argument(
        "--load_path",
...
def main():
    ...
    # transE_weight = torch.load("8_TransR/model_save/TransE-model-35.pth")
    # with torch.no_grad():
