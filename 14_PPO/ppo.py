...
# %%
name_actor_model = "./model_save/base_model/llama-2-7b"
...
def batched_forward_pass(
    model_actor, model_value, input_ids, attention_mask, ref=False
):
    if ref:
        model_actor.eval()
    else:
...
def get_data(
    ...
    for batch in tqdm(dataloader, desc="Generate data"):
        ...
        kl = (prob_log_old - prob_log_ref) ** 2 * 0.5 * -0.2  # reward = kl + score
    ...
    for _ in range(epochs):
        ...
        for batch in tqdm(dataloader, desc="Training step"):
            ...
            prob_log_new, value_new = batched_forward_pass(
                model_actor, model_value, input_ids[0], attention_mask, ref=False
            )
            # 重要性采样
...
try:
    ...
    model_actor.save_pretrained(
        "model_save/ppo_model/llama-2-7b", safe_serialization=True
    )
# %%
torch.cuda.empty_cache()
